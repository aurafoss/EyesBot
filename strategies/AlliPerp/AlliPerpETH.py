...
if len(ftx.get_open_position(perpSymbol)) == 0:
    ...
    if openLongCondition(df.iloc[-2]):
        ...
        print("AlliPerpETH:Open a market LONG at", actualPrice)
        ...
        print("AlliPerpETH:Place a Stop Loss at ", actualPrice - 0.03 * actualPrice)
        ...
        print("AlliPerpETH:Open a market SHORT at", actualPrice)
        # -- Create a market stop loss -3% --
        stopLoss = ftx.place_market_stop_loss(
            perpSymbol, 
            'buy', 
        ...
        print("AlliPerpETH:No opportunity to take")
    ...
    if ftx.get_open_position(perpSymbol)[0]['side'] == 'long':
        ...
        if closeLongCondition(df.iloc[-2]):
            ...
            ftx.cancel_all_open_order(perpSymbol)
            print('AlliPerpETH:Close my LONG position')
        else:
            print("AlliPerpETH:A LONG is running and I don't want to stop it")
    # -- Check if you have a SHORT open --
    elif ftx.get_open_position(perpSymbol)[0]['side'] == 'short':
        if closeShortCondition(df.iloc[-2]):
            ftx.close_all_open_position(perpSymbol)
            ftx.cancel_all_open_order(perpSymbol)
            print('AlliPerpETH:Close my SHORT position')
        else:
            print("AlliPerpETH:A SHORT is running and I don't want to stop it")