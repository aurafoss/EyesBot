...
print("CrossEMA+RSI:Execution Time :", current_time)
...
for order in open_orders:
    ...
    if float(order["filledSize"]) > 0:
        print(
            f"CrossEMA+RSI:Order on {order['market']} is partially fill, create {order['side']} Market of {order['remainingSize']} {order['market']} order to complete it"
        )
...
for pair in pair_to_check:
    ...
    if row['ema1'] > row['ema2'] and row['stoch_rsi'] < 0.8 and row['close'] > row['sma_long']:
        ...
        print(
            f"CrossEMA+RSI:Place Buy Limit Order: {buy_quantity} {pair[:-4]} at the price of {buy_limit_price}$ ~{round(exchange_buy_quantity, 2)}$"
        )
...
for pair in positions:
    ...
    if row['ema2'] > row['ema1'] and row['stoch_rsi'] > 0.2:#definition d'ordre de vente
        ...
        print(
            f"CrossEMA+RSI:Place Sell Limit Order: {sell_quantity} {pair[:-4]} at the price of {sell_limit_price}$ ~{round(exchange_sell_quantity, 2)}$"
        )
...
print("CrossEMA+RSI:My current balance in USD:", new_coin_in_usd)